...
export default function Orders() {
  ...
  const filteredOrders = orderData.filter(order => {
    ...
    const matchesSearch =
      ...
      order.CustomerFirstName.toLowerCase().includes(searchText.toLowerCase()) ||
      order.CustomerLastName.toLowerCase().includes(searchText.toLowerCase());
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: 'grey' }}>
      <View style={styles.headerContainer}>
        <ThemedText type="title" style={{ color: 'black', fontStyle: 'Bold', paddingTop: 10, paddingBottom: 5 }}>
          Order Management
        </ThemedText>
      </View>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 10, marginBottom: 10 }}>
      ...
      <View style={{ flex: 1, position: 'relative' }}>
        ...
        >
          ...
          <Pressable
            style={styles.addButton}
            onPress={() => router.push('/(tabs)/orders/order_calendar')}>
            <Text> calendar </Text>
          </Pressable>
        </View>
      </View>
